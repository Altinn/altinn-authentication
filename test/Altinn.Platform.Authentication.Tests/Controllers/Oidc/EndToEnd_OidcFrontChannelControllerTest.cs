...
{
    /// <summary>
    /// End-to-end front-channel tests for <see cref="Authentication.Controllers.OidcFrontChannelController"/>
    /// covering the full OIDC authorization-server flow (authorize → upstream callback → token → refresh → logout).
    /// </summary>
    ...
    .ReturnsAsync(true);
        ...
        [Fact]
        public async Task TC13_Auth_IdPortenEmail_NewUser_Vanskeligepost_Aa_App_Af_App_Af_Logout_End_To_End_OK()
        {
            // Create HttpClient with default headers for IP, UA, correlation. 
            using HttpClient client = CreateClientWithHeaders();
            OidcTestScenario testScenario = OidcScenarioHelper.GetScenario("Arbeidsflate_EpostBruker_VanskeligEpost");
