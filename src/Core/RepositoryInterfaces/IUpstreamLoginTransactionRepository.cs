...
namespace Altinn.Platform.Authentication.Core.RepositoryInterfaces
{
    ...
    {
        Task<UpstreamLoginTransaction> InsertAsync(UpstreamLoginTransactionCreate create, CancellationToken cancellationToken = default);

        /// <summary>Finds a pending/callback_received row by upstream <c>state</c> (used in callback).</summary>
        Task<UpstreamLoginTransaction?> GetForCallbackByStateAsync(string upstreamState, CancellationToken cancellationToken = default);
        ...
        Task<int> SetCallbackSuccessAsync(Guid upstreamRequestId, string authCode, DateTimeOffset receivedAt, CancellationToken cancellationToken = default);

        /// <summary>Stores the upstream error at callback and moves status to <c>error</c>.</summary>
        Task<int> SetCallbackErrorAsync(Guid upstreamRequestId, string error, string? errorDescription, DateTimeOffset receivedAt, CancellationToken cancellationToken = default);
        ...
        Task<int> MarkCompletedAsync(Guid upstreamRequestId, bool success, DateTimeOffset completedAt, CancellationToken cancellationToken = default);
