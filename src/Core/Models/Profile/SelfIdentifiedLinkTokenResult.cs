...
namespace Altinn.Platform.Authentication.Core.Models.Profile
{
    ...
    /// <see cref="TargetUserId"/> (the SI user being claimed).
    /// </summary>
    public class SelfIdentifiedLinkTokenResult
    {
        ...
        /// <summary>
        /// Gets the self-identified user being claimed (token <c>target_user_id</c>).
        /// </summary>
        ...
        /// <summary>
        /// Creates a successful result with the bound source/target user ids and token id.
        /// </summary>
        public static SelfIdentifiedLinkTokenResult Valid(int sourceUserId, int targetUserId, string? tokenId) =>
            new() { IsValid = true, SourceUserId = sourceUserId, TargetUserId = targetUserId, TokenId = tokenId };
    }
}
