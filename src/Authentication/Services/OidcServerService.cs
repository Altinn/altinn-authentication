...
namespace Altinn.Platform.Authentication.Services
{
    ...
    CancellationToken cancellationToken)
        ...
        private async Task<(OidcSession OidcSession, string SessionHandle)> CreateOrUpdateOidcSession(UpstreamLoginTransaction upstreamTx, UserAuthenticationModel userIdenity, CancellationToken cancellationToken)
        {
            ...
            {
                externalId = $"{AltinnCoreClaimTypes.IdPortenEmailPrefix}:{UrnEncoded.Create(userIdenity.Email.ToLowerInvariant()).Value}";
            }
        ...
        private async Task<UserAuthenticationModel> IdentifyOrCreateAltinnUser(UserAuthenticationModel userAuthenticationModel, OidcProvider? provider)
        {
            ...
            else if (userAuthenticationModel.Acr != null && userAuthenticationModel.Acr.Equals("selfregistered-email") && !string.IsNullOrEmpty(userAuthenticationModel.Email))
            {
                ...
                string issExternalIdentity = AltinnCoreClaimTypes.IdPortenEmailPrefix + ":" + UrnEncoded.Create(userAuthenticationModel.Email.ToLowerInvariant()).Value;
                userProfile = await _userProfileService.GetUser(issExternalIdentity);
