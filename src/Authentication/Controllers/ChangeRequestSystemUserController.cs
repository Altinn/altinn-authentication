    ...
    IOptions<GeneralSettings> generalSettings) : ControllerBase
{
    ...
    /// or a combination of the owner's Orgno + Registered SystemId + (optionally) External Reference.    
    /// </summary>
    /// <param name="correllationId">REQUIRED: A new UUID (RFC 4122) generated by the Vendor, should not be the same as the SystemUserId, 
    /// used to uniquely identify the Change Request throughout it's entire lifecylce. Don't set it equal to the SystemUser Id!</param>
    /// <param name="systemUserId">OPTIONAL: The UUID (RFC 4122) of the targeted SystemUser issued by us, if known to the caller. 
    /// Otherwise use the combination of orgno + systemid</param>
    /// <param name="reporteeOrgno">OPTIONAL: The Organisation Number for the Reportee (the customer) that owns the SystemUser. (Not the Vendor!)</param>    
    /// <param name="systemId">OPTIONALThe Id for the Registered System that the SystemUser is based on.</param>
    /// <param name="externalRef">OPTIONAL: If the SystemUser was created with an ExternalRef, set this, otherwise leave blank.</param>
    ...
    public async Task<ActionResult<ChangeRequestResponse>> CreateChangeRequest(
        ...
        [FromQuery(Name = "orgno")] string? reporteeOrgno,        
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!systemUserId.HasValue && reporteeOrgno is not null && systemId is not null)
        {
            ExternalRequestId externalRequestId = new()
            {
                ExternalRef = externalRef ?? reporteeOrgno,
                OrgNo = reporteeOrgno,
                SystemId = systemId,
            };
        ...
        if (correllationId == new Guid(systemUser.Id))
        {
            return BadRequest("The correlation-id should not be the same as the system-user-id. Generate a unique id.");
        }
        ...
        if (emptyResponseTest)
        {
            ChangeRequestResponse emptyResponse = new()
            {
                ...
                PartyOrgNo = reporteeOrgno ?? systemUser.ReporteeOrgNo
            };        
